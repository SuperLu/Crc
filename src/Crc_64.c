...
#ifndef Crc_64_Mode
#define Crc_64_Mode 0
#endif


/* Check that the CRC 64 configuration is consistent */
#if Crc_64_Mode == CRC_64_HARDWARE
#error "Crc_64_Mode is set to CRC_64_HARDWARE which isn't supported"
#endif


/* For CRC64, https://en.wikipedia.org/wiki/Cyclic_redundancy_check */
#ifdef Crc_64_Mode
#define CRC_64_StartValue    0xFFFFFFFFFFFFFFFFU
 ...
 */
static inline uint64_t reflectResult(uint64_t data)
{
    uint64_t reflection = 0x0000000000000000U;
    uint8_t bit;
 ...
 *  @return reflected data
 */
static inline uint8_t reflectInData(uint8_t data)
{
...
/** @brief Calculation of 64-bit CRC,
 ...
 */
static uint64_t calculateCRC64(const uint8_t* message, uint32_t nBytes, uint64_t start)
{
    uint64_t remainder = reflectResult(start);
    uint8_t  bit;
    const uint64_t  topbit = 0x8000000000000000U;
    ...
    if (message != NULL) {
        ...
        for (uint32_t byte = 0; byte < nBytes; byte++) {
            ...
            uint64_t reflectedData = reflectInData(*message);
            remainder ^= (reflectedData << 56u);
            message++;
            ...
            for (bit = 8; bit > 0; bit--) {
                ...
                if ((remainder & topbit) != 0) {
                    remainder = (remainder << 1u) ^ CRC_64_Polynomial;
                }
                else {
...
uint64_t Crc_CalculateCRC64(const uint8_t* Crc_DataPtr, uint32_t Crc_Length, uint64_t Crc_StartValue64, bool Crc_IsFirstCall) {
    ...
    uint64_t crc = 0; /* Default return value if NULL pointer */
#if Crc_64_Mode == CRC_64_TABLE
    static const uint64_t CRC_64_Tab[]  = {0x0000000000000000,0xb32e4cbe03a75f6f,0xf4843657a840a05b,0x47aa7ae9abe7ff34,0x7bd0c384ff8f5e33,0xc8fe8f3afc28015c,0x8f54f5d357cffe68,0x3c7ab96d5468a107,
    ...
    if (Crc_DataPtr != NULL) {
        ...
        crc = (true == Crc_IsFirstCall) ? CRC_64_StartValue : (Crc_StartValue64 ^ CRC_64_Xor);

#if Crc_64_Mode == CRC_64_RUNTIME
        crc = calculateCRC64(Crc_DataPtr, Crc_Length, crc);
#elif Crc_64_Mode == CRC_64_TABLE
        for( uint32_t byte = 0; byte < Crc_Length; byte++) {
            crc = ((crc >> 8) & 0x00FFFFFFFFFFFFFFU) ^ CRC_64_Tab[(crc ^ *Crc_DataPtr) & 0xFFU];
            Crc_DataPtr++;
        }
#endif
