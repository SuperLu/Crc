 ...
 */
static inline uint32_t reflectResult(uint32_t data)
{
    uint32_t reflection = 0x00000000U;
    uint8_t bit;
 ...
 */
static uint32_t calculateCRC32P4(const uint8_t* message, uint32_t nBytes, uint32_t start)
{
    uint32_t remainder = reflectResult(start);
    uint8_t  bit;
    ...
    if (message != NULL) {
        ...
        for (uint32_t byte = 0; byte < nBytes; byte++) {
            ...
            uint32_t reflectedData = reflectInData(*message);
...
uint32_t Crc_CalculateCRC32P4(const uint8_t* Crc_DataPtr, uint32_t Crc_Length, uint32_t Crc_StartValue32, bool Crc_IsFirstCall) {
    ...
    static const uint32_t Crc_32P4_Tab[] = {  0x00000000U, 0x30850FF5U, 0x610A1FEAU, 0x518F101FU, 0xC2143FD4U, 0xF2913021U, 0xA31E203EU, 0x939B2FCBU,
